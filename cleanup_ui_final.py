...
v4_script_style = """    <script>
        ...
        // Missing Health Check and Mode Toggle from original script we might need?
        // Let's add them back minimally.
        ...
        document.addEventListener('DOMContentLoaded', function () {
            checkHealth();
            // loadIndexStats(); // Not needed if we use main stats
        ...
        async function checkHealth() {
            ...
            try {
                ...
                // Update specific status elements if they exist
                const os = document.getElementById('ollama-status');
                if (os) os.innerText = data.ollama?.available ? '✓ Online' : '✗ Offline';
...
# TRUNCATION STRATEGY:
# Find </main> and </div> that closes the main content logic.
...
if split_marker in content:
    ...
    # We take the first part (including </main> implicitly if we add it back, or just before it)
    # Actually split removes the marker.
    
    # Let's verify the markup around </main>.
    # It is usually followed by </div>.
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully truncated and replaced scripts in index.html")
else:
    print("Error: Could not find split marker </main> in index.html")
